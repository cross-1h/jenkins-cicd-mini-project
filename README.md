...
This project builds the CI side: get the code, install dependencies, run tests, and package the app into a Docker image. That is the four-stage pipeline you will build. The CD side, pushing that image to a registry and deploying it so users can reach it, is the next project. See `pipeline-diagram.svg`, where the four stages you build are solid and the delivery steps are faded to show what comes next.
